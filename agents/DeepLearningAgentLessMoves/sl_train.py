...
def train_supervised():
    ...
    for epoch in range(EPOCHS):
        ...
        for batch_idx, (b, p_target, v_target) in enumerate(dataloader):
            ...
            # --- WEIGHTED LOSS LOGIC ---
            # Calculate how full the board is
            stones = torch.sum(b[:, 0:2, :, :], dim=(1, 2, 3)) 
            total_spots = 11 * 11
            fullness = stones / total_spots 

            # Create weight: End-game positions get higher importance
            weights = 1.0 + (fullness ** 4) 

            # Apply weight to policy loss
            loss_p = -torch.sum(p_target * p_pred, dim=1) # Sum over moves, keep batch dim
            loss_p = (loss_p * weights).mean() # Weighted average
